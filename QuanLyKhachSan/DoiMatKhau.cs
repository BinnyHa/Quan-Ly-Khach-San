        ...
        private void btnDoiMK_Click(object sender, EventArgs e)
        {
            if ((MessageBox.Show("Xác nhận thay đổi", "Đổi mật khẩu", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes))
            {
                ...
                if ((dta.Read() == true && dta.GetValue(0).ToString() != "") || UserInfo.ChucVu == "Quản Lý")
                {
                    ...
                    //else if(!Regex.IsMatch(txtMK2.Text, @"([A-Z])"))
                    //{
                    //    MessageBox.Show("Phải có ký tự viết hoa");
                    //}
                    //else if (!Regex.IsMatch(txtMK2.Text, @"([a-z])"))
                    //{
                    //    MessageBox.Show("Phải có ký tự thường");
                    //}
                    //else if (!Regex.IsMatch(txtMK2.Text, @"([0-9])"))
                    //{
                    //    MessageBox.Show("Phải có ký tự số");
                    //}
                    else
                    {
                        con.Open();
