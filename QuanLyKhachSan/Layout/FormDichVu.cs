...
namespace QuanLyKhachSan.Layout
{
    ...
    adapt.Fill(dt);
        ...
        private void dataDichVu_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (dataDichVu.CurrentCell.ColumnIndex.Equals(4) && e.RowIndex != -1)
            {
                con.Open();
                if (dataDichVu.CurrentCell != null && dataDichVu.CurrentCell.Value != null)
                {
            ...
            else if (dataDichVu.CurrentCell.ColumnIndex.Equals(3) && e.RowIndex != -1)
            {
                if (dataDichVu.CurrentCell != null && dataDichVu.CurrentCell.Value != null)
                {
                    if (dataDichVu.Rows[e.RowIndex].Cells[3].Value.ToString() == "True")
                    {
        ...
        {
            SubLayout.ChiTietDV ctdv = new SubLayout.ChiTietDV();
            ctdv.Show();
        ...
        private void btnTimkiem_Click(object sender, EventArgs e)
        {
            con.Open();
            DataTable dt = new DataTable();
            adapt = new SqlDataAdapter("SELECT * FROM dbo.DICHVU  where MADICHVU like N'%" + txtTimkiem.Text +
                "%' or TENDICHVU like N'%" + txtTimkiem.Text + "%' or GIATIEN like N'%" + txtTimkiem.Text + "%'", con);
            adapt.Fill(dt);
            dataDichVu.DataSource = dt;
            con.Close();
        }
